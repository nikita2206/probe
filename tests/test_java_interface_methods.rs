...
fn test_java_interface_method_declarations_appear_in_search_results() {
    ...
    let output = cmd.current_dir(temp_path)
        .arg("getUserById")
        .output()
    ...
    assert!(stdout.contains("getUserById"), "Method name should be found in search results");
    
    // Verify that the method declaration is included in the results
    assert!(stdout.contains("User getUserById(String userId)"), 
            "Method declaration should appear in search results");
    
    // Additional checks to ensure we get the full context
    assert!(stdout.contains("UserService.java"), "File name should be mentioned");
    
    // The method should be identified as a Method
    assert!(stdout.contains("Method getUserById"), "Should identify this as a method");
}
...
fn test_java_interface_multiple_methods_with_same_pattern() {
    ...
    assert!(stdout.contains("createUser"), "Should find createUser method");
    
    // Verify method declarations are visible in results
    assert!(stdout.contains("User createUser(String username, String email)"),
...
fn test_java_class_methods_work_correctly() {
    ...
    assert!(stdout.contains("public String getId()"), "Should show method implementation");
