...
"#, lang_full);
        ...
        let config = Config::load_from_dir(temp_dir.path()).unwrap();
        assert!(config.get_language().is_ok(), "Should support language: {}", lang_full);
...
"#, lang_short);
        ...
        let config = Config::load_from_dir(temp_dir.path()).unwrap();
        assert!(config.get_language().is_ok(), "Should support language code: {}", lang_short);
...
"#;
    ...
    let config = Config::load_from_dir(temp_dir.path()).unwrap();
    assert!(config.get_language().is_err(), "Should reject invalid language");
