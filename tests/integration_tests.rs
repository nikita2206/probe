...
fn get_test_data_path() -> &'static str {
    concat!(env!("CARGO_MANIFEST_DIR"), "/tests/test_data/sample_project")
}
...
fn get_gitignore_test_data_path() -> &'static str {
    concat!(env!("CARGO_MANIFEST_DIR"), "/tests/test_data/gitignore_test")
}
...
fn test_incremental_update() {
    ...
    // Add a new file
    fs::write(project_path.join("new_file.rs"), 
...
fn test_help_command() {
    Command::cargo_bin("codesearch")
        ...
        .assert()
        .success()
        .stdout(predicate::str::contains("Fast code search with persistent indexing"))
...
fn test_filetype_filtering() {
    ...
    Command::cargo_bin("codesearch")
        .unwrap()
        .args(&["-d", project_path.to_str().unwrap(), "-t", "json", "database_url"])
        .assert()
        .success()
        .stdout(predicate::str::contains("config.json"));
...
fn test_filetype_filtering_markdown() {
    ...
    Command::cargo_bin("codesearch")
        .unwrap()
        .args(&["-d", project_path.to_str().unwrap(), "-t", "md", "Sample Project"])
        .assert()
        .success()
